...
export class ClueManager {
  ...
  unlockClueForAll(clueId: string) {
    ...
    // for (const team of teams) {
      ...
      /* teamClient &&  */game.sendCluesToClients()
    // }
  ...
  assignRandomMainClueType() {
    ...
    // const teams = game.getTeam().map(team => team.id)
    const team = game.getTeam()
    ...
    let numberOfDiaryClues = Object.values(this.mainClueType).filter(type => type === 'diary').length
      ...
      if (randomValue < chance) {
        this.mainClueType[team.id] = 'phone'
        numberOfPhoneClues++
      } else {
        this.mainClueType[team.id] = 'diary'
        numberOfDiaryClues++
      }

      console.log(colorize('[ClueManager]', Fg.Blue), 'Main clue type assigned randomly to team', team.id, this.mainClueType[team.id])
    // }
