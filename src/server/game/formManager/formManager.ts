...
import { getFieldFromId } from '../../../shared/assets/form'
...
export class FormManager {
  ...
  public clean (): void {
    ...
    this.submittedForms = this.submittedForms.filter(() => game.getTeam())
  ...
  public setFormPage(teamId: string, page: number): void {
    ...
    Game.get().sendFormPageToTeamClient()
    // Game.get().sendFormsToAdmins()
    this.save()
  }
  ...
  public getAllFormsSubmitted (): boolean {
    return [Game.get().getTeam()].filter(team => team.active).every(team => this.submittedForms.includes(team.id))
  }
