<template>
  <div class="sus-db__wrapper" ref="database">
        ...
        <div class="sus-db__header">
          ...
          <div class="sus-db__help-btn">
            <HelpBtn>
              ...
              <p>
                Details zu den markierten Hinweisen können durch Klicken auf den Hinweis eingesehen werden.
              </p>
            </HelpBtn>
          </div>
        </div>
            ...
            <Transition :name="'sus-db__entries__tab--' + suspectTransition">
              ...
              >
                <TransitionGroup name="sus-db__entries__entry" @enter="enter" :css="false">
                  <DatabaseEntry
                    :scroller="entryTabs[suspects.findIndex(s => s.id === activeSuspect)]"
                ...
                <VFadeTransition>
                  <div v-if="(delayedEntries[suspect.id]?.length ?? 0) === 0" class="sus-db__entries__no-entries">
                    <span>
...
const suspects = computed<(Suspect & {
  ...
  suspects.push(...game.shownSuspects.map(suspectId => allSuspects.find(suspect => suspect.id === suspectId)).filter(Boolean) as Suspect[]);

  Object.keys(entries.value).forEach((suspectId) => {
    const suspect = allSuspects.find(suspect => suspect.id === suspectId);

    if (suspect && suspects.findIndex(s => s.id === suspectId) === -1) {
...
const database = ref<HTMLElement | null>(null);

watch(database, (el) => {
  if (el) {
    tutorial.registerHighlightElement('database', {
      element: el,
      borderRadius: 8,
      margin: -8
    });
  } else {
    tutorial.unregisterHighlightElement('database');
  }
}, { immediate: true });
